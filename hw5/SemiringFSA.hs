...
backwardWeight :: GenericAutomaton st sy Float -> [sy] -> st -> Float
backwardWeight m w q =
    let (states, syms, i, f, delta) = m in
    case w of
    [] -> f q
    (x:rest) -> maximum (map (\q1 -> delta (q,x,q1) * backwardWeight m rest q1) states)

fWeight :: GenericAutomaton st sy Float -> [sy] -> Float
fWeight m w =
    let (states, syms, i, f, delta) = m in
    maximum (map (\q -> i q * backwardWeight m w q) states)
